...
// Gap Feature: Accounts without \'/net
// Accounts without /net-worth-forecast (project wealth over time)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / gap-accounts-without-net',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Accounts without \'/net'.\nProject: AIPersonalFinanceMultiAgent.\nContext: Accounts without /net-worth-forecast (project wealth over time)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['accounts-without-net', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'accounts-without-net', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

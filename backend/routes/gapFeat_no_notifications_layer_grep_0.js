...
// Gap Feature: No notifications layer (grep 0)
// No notifications layer (grep 0)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / gap-no-notifications-layer-grep-0',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No notifications layer (grep 0)'.\nProject: AIPersonalFinanceMultiAgent.\nContext: No notifications layer (grep 0)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-notifications-layer-grep-0', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-notifications-layer-grep-0', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

...
// Gap Feature: Budgets without \'/budget
// Budgets without /budget-recommend (optimize allocation based on spending patterns)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / gap-budgets-without-budget',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Budgets without \'/budget'.\nProject: AIPersonalFinanceMultiAgent.\nContext: Budgets without /budget-recommend (optimize allocation based on spending patterns)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['budgets-without-budget', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'budgets-without-budget', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

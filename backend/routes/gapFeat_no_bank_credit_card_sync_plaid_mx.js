...
// Gap Feature: No bank/credit card sync (Plaid, MX)
// No bank/credit card sync (Plaid, MX) — only manual entry
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / gap-no-bank-credit-card-sync-plaid-mx',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No bank/credit card sync (Plaid, MX)'.\nProject: AIPersonalFinanceMultiAgent.\nContext: No bank/credit card sync (Plaid, MX) — only manual entry\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-bank-credit-card-sync-plaid-mx', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-bank-credit-card-sync-plaid-mx', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-bank-credit-card-sync-plaid-mx']
    );

...
// Gap Feature: Limited spending analytics (no aggregations beyond CRUD)
// Limited spending analytics (no aggregations beyond CRUD)
// Project: AIPersonalFinanceMultiAgent
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / gap-limited-spending-analytics-no-aggregations-beyond-',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited spending analytics (no aggregations beyond CRUD)'.\nProject: AIPersonalFinanceMultiAgent.\nContext: Limited spending analytics (no aggregations beyond CRUD)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-spending-analytics-no-aggregations-beyond-', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-spending-analytics-no-aggregations-beyond-', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-spending-analytics-no-aggregations-beyond-']
    );

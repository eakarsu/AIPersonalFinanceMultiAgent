...
// Predict when money will run out; recommend when to pay bills to maximize float
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / cf-cashflow-forecasting',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_cashflow_forecasting`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-cashflow-forecasting feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Cashflow forecasting'.
Context: Predict when money will run out; recommend when to pay bills to maximize float
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-cashflow-forecasting/run error:', err);

...
// Multi-goal optimization (debt payoff vs. savings vs. investing); AI suggests spending priorities given constraints
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / cf-financial-goal-orchestration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_financial_goal_orchestration`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-financial-goal-orchestration feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Financial goal orchestration'.
Context: Multi-goal optimization (debt payoff vs. savings vs. investing); AI suggests spending priorities given constraints
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-financial-goal-orchestration/run error:', err);

...
// Scan bank statements for recurring charges; flag low-usage subscriptions; recommend cancellation or downgrades; track annual savings
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / cf-subscription-analyzer',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_subscription_analyzer`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-subscription-analyzer feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Subscription analyzer'.
Context: Scan bank statements for recurring charges; flag low-usage subscriptions; recommend cancellation or downgrades; track annual savings
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-subscription-analyzer/run error:', err);

...
// Background process continuously monitors spending; auto-rebalances budget allocations if trending over; alerts user on pace to overspend
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / cf-autonomous-budget-agent',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_autonomous_budget_agent`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-autonomous-budget-agent feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Autonomous budget agent'.
Context: Background process continuously monitors spending; auto-rebalances budget allocations if trending over; alerts user on pace to overspend
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-autonomous-budget-agent/run error:', err);

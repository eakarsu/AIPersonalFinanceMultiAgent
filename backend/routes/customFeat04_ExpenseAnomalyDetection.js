...
// Real-time streaming; flag unusual transactions (merchant category, amount, location); prevent fraud
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalFinanceMultiAgent / cf-expense-anomaly-detection',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_expense_anomaly_detection`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-expense-anomaly-detection feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Expense anomaly detection'.
Context: Real-time streaming; flag unusual transactions (merchant category, amount, location); prevent fraud
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-expense-anomaly-detection/run error:', err);

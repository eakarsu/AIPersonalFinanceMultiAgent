...
// Custom Feature: Cashflow forecasting
// Cashflow forecasting: Predict when money will run out; recommend when to pay bills to maximize float
import React, { useState } from 'react';

export default function CFCashflowForecastingPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-cashflow-forecasting/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Cashflow forecasting</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Cashflow forecasting: Predict when money will run out; recommend when to pay bills to maximize float</p>

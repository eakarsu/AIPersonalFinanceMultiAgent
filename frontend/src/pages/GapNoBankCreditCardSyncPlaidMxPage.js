...
// Gap (Non-AI): No bank/credit card sync (Plaid, MX)
// No bank/credit card sync (Plaid, MX) — only manual entry
import React, { useState } from 'react';

export default function GapNoBankCreditCardSyncPlaidMxPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-bank-credit-card-sync-plaid-mx/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No bank/credit card sync (Plaid, MX)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No bank/credit card sync (Plaid, MX) — only manual entry</p>

...
// Gap (AI): Accounts without '/net
// Accounts without /net-worth-forecast (project wealth over time)
import React, { useState } from 'react';

export default function GapAccountsWithoutNetPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-accounts-without-net/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Accounts without '/net</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Accounts without /net-worth-forecast (project wealth over time)</p>

...
// Gap (AI): Subscriptions without '/subscription
// Subscriptions without /subscription-audit (identify unused subscriptions)
import React, { useState } from 'react';

export default function GapSubscriptionsWithoutSubscriptionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-subscriptions-without-subscription/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Subscriptions without '/subscription</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Subscriptions without /subscription-audit (identify unused subscriptions)</p>

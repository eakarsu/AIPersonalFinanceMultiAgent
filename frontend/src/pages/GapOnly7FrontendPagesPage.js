...
// Gap (Non-AI): Only 7 frontend pages
// Only 7 frontend pages — limited coverage
import React, { useState } from 'react';

export default function GapOnly7FrontendPagesPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-only-7-frontend-pages/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Only 7 frontend pages</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Only 7 frontend pages — limited coverage</p>

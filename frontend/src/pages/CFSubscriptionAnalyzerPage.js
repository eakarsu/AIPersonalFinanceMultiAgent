...
// Custom Feature: Subscription analyzer
// Subscription analyzer: Scan bank statements for recurring charges; flag low-usage subscriptions; recommend cancellation or downgrades; track annual savings
import React, { useState } from 'react';

export default function CFSubscriptionAnalyzerPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-subscription-analyzer/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Subscription analyzer</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Subscription analyzer: Scan bank statements for recurring charges; flag low-usage subscriptions; recommend cancellation or downgrades; track annual savings</p>

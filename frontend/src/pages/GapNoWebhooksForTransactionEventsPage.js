...
// Gap (Non-AI): No webhooks for transaction events
// No webhooks for transaction events
import React, { useState } from 'react';

export default function GapNoWebhooksForTransactionEventsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-transaction-events/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for transaction events</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for transaction events</p>

...
export default function GapBudgetsWithoutBudgetPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-budgets-without-budget/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Budgets without '/budget</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Budgets without /budget-recommend (optimize allocation based on spending patterns)</p>

...
// Custom Feature: Expense anomaly detection
// Expense anomaly detection: Real-time streaming; flag unusual transactions (merchant category, amount, location); prevent fraud
import React, { useState } from 'react';

export default function CFExpenseAnomalyDetectionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-expense-anomaly-detection/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Expense anomaly detection</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Expense anomaly detection: Real-time streaming; flag unusual transactions (merchant category, amount, location); prevent fraud</p>
